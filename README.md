...
Use the arrow keys to control the snake. Eat food (`*`) to grow longer. Avoid running into the walls, your own body, or the moving enemy (`X`).
Press `Ctrl` at any time for a short speed boost. After the game ends, press `r` to restart or `q` to quit.
